...
class TaskPickUpTranslationController extends Controller
{
    /**
    ...
    public function store(Request $request)
    {
        ...
        $pickUpTranslationTask = TaskPickUpTranslation::create($pickUpTranslation);
        ...
        foreach ($translations as $translation) {
            $translationsData = [
                ...
                'pick_up_translation_id' => $pickUpTranslationTask->id
            ];

            TaskTranslation::create($translationsData);
        }

        return response()->json([
            'task' => new TaskResource($task),
            'pick_up_translation' => $pickUpTranslationTask
        ], 200);
    }
    ...
    public function show($id)
    {
        ...
        $pickUpTranslation = TaskPickUpTranslation::where('task_id', $id)->first();

        return response()->json([
            'task' => new TaskResource($task),
            'pick_up_translation' => $pickUpTranslation
        ], 200);
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        TaskPickUpTranslation::withTrashed()->where('task_id', $id)->update($pickUpTranslation);

        $pickUpTranslationTask = TaskPickUpTranslation::withTrashed()->where('task_id', $id)->first();
        ...
        foreach ($translations as $translation) {
            TaskTranslation::updateOrCreate(
                ['id' => $translation['id']],
                ['translation' => $translation['translation'], 'phrase' => $translation['phrase'], 'pick_up_translation_id' => $pickUpTranslationTask->id]
            );
        }
    ...
    {
        $test_task_file = TaskFile::findOrFail($id);
        $test_task_file->delete();

        return response()->json([
            'status' => true,
            'message' => 'Task File has been deleted successfully!'
        ], 200);
    }
    ...
    {
        $translations_id = $request->translations_id;

        TaskTranslation::withTrashed()->whereIn('id', $translations_id)->delete();

        return response()->json([
            'status' => true,
            'message' => 'Translations has been deleted successfully!'
        ], 200);
    }
}
